...
TEST_CASE("test_aggregator_cycle")
{
  ...
  qoMap["checkAll"] = qo1;
  qo1->addMetadata(o2::quality_control::repository::metadata_keys::cycle, "1");
  auto qo2 = make_shared<QualityObject>(Quality::Bad, "dataSizeCheck2/skeletonTask/example");
  qo2->addMetadata(o2::quality_control::repository::metadata_keys::cycle, "2");
  qoMap["dataSizeCheck2/skeletonTask/example"] = qo2;
  auto results = aggregator->aggregate(qoMap);
  for (auto r : results) {
    REQUIRE_NOTHROW(r->getMetadata(o2::quality_control::repository::metadata_keys::cycle));
    CHECK(r->getMetadata(o2::quality_control::repository::metadata_keys::cycle) == "2");
  }
}
